...
N = [n];
d = 3;
M = [4,10,12,18,20,22,24];
Ratio = [0.1,0.24,0.29,0.36,0.41,0.47,0.55];
...
for i=1:length(N)
    ...
    for j=1:length(r_minus_d_list)
        ...
        % Graph 5
        cost_relax_list5 = zeros(num_exp,length(M(i,:)));
        cost_proj_list5 = zeros(num_exp,length(M(i,:)));
        m_list5 = zeros(num_exp,length(Ratio));
        ...
        for k=1:length(M(i,:))
            ...
            for l=1:num_exp
                ...
                R=1;
                r=0.5;
                [X, G, C, D, edges, A] = graphTorus3D(n,m,R,r,false);
                m = size(D,1);
                m_list5(l,k) = m;
                ...
                [x, Y, xcost, info, info_path_optimization] = DistanceBasedOptimization(D,C,W,d,r_minus_d,init,Y_0,option);
                cost_relax_list5(l,k) = abs(xcost);
        ...
        if r_minus_d > 0
           ...
           [mean_,ind] = sort(mean(m_list3,1));
            ...
            [mean_,ind] = sort(mean(m_list5,1));
            mean_cost = mean(cost_proj_list5,1);
            semilogy(mean_,mean_cost(ind),'-v','DisplayName','Local structure on the torus');

            legend();
            xlabel('Number of edges');
            ylabel('Cost');
            hold off;
            name = strcat('FigData/Dimension3/mean_cost_proj_n_',int2str(n),'_rmd_',int2str(r_minus_d),'.png')
            saveas(fig,name);
       ...
       [mean_,ind] = sort(mean(m_list3,1));
        ...
        [mean_,ind] = sort(mean(m_list5,1));
        mean_cost = mean(cost_relax_list5,1);
        semilogy(mean_,mean_cost(ind),'-v','DisplayName','Local structure on the torus');

        legend();
        xlabel('Number of edges');
        ylabel('Cost');
        name = strcat('FigData/Dimension3/mean_cost_relax_n_',int2str(n),'_rmd_',int2str(r_minus_d),'.png')
        hold off;
        saveas(fig,name);
        ...
        if r_minus_d > 0
           ...
           [mean_,ind] = sort(mean(m_list3,1));
            ...
            [mean_,ind] = sort(mean(m_list5,1));
            mean_cost = median(cost_proj_list5,1);
            semilogy(mean_,mean_cost(ind),'-v','DisplayName','Local structure on the torus');
            ...
            ylabel('Cost');
            hold off;
            name = strcat('FigData/Dimension3/median_cost_proj_n_',int2str(n),'_rmd_',int2str(r_minus_d),'.png');
            saveas(fig,name);
       ...
       [mean_,ind] = sort(mean(m_list3,1));
        ...
        [mean_,ind] = sort(mean(m_list5,1));
        mean_cost = median(cost_relax_list5,1);
        semilogy(mean_,mean_cost(ind),'-v','DisplayName','Local structure on the torus');
        ...
        ylabel('Cost');
        hold off;
        name = strcat('FigData/Dimension3/median_cost_relax_n_',int2str(n),'_rmd_',int2str(r_minus_d),'.png');
        saveas(fig,name);
