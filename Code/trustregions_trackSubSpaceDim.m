function [x, cost, info, options, info_path_optimization] = trustregions_trackSubSpaceDim(problem, x, options,d,r,cost_fct)
...
while true
    ...
    if model_decreased && rho > options.rho_prime
        ...
        % Store informations  for each iterate 
        [U_x, D_x, V_x] = svd(x);
        info_path_optimization{k+1}.singular_val = diag(D_x);
        info_path_optimization{k+1}.right_singular_space = V_x;
        info_path_optimization{k+1}.cost = fx;
        info_path_optimization{k+1}.normGrad = norm_grad;
        info_path_optimization{k+1}.grad = fgradx;

        info_path_optimization{k+1}.cost_proj = cost_fct(normr(U_x*D_x(:,1:d)));
